...
void signup() // Function to create a new user for the Food ordering system
{
	...
	x = validate(); // Function Call to validate the user creation
	if (x == 1)
		account_check();
}

int validate() // Function to validate the user for signup process
{
	// Validate the name
...
void hotel_initialize()
{
	...
	strcpy(m[1].hotel, " Hotel_Vaishali");
	...
	strcpy(m[2].hotel, "Hotel_Annapurna");
	strcpy(m[2].first_food, "Parotta");
	strcpy(m[2].second_food, "Noodles");
	strcpy(m[2].third_food, "Manchurian");
	m[2].first = 15;
	m[2].second = 75;
	m[2].third = 80;
	...
	strcpy(m[3].third_food, "Mutton_Biryani");
	m[3].first = 90;
	m[3].second = 120;
	m[3].third = 35;
}
